...
const Drivers: React.FC = () => {
  ...
  const fetchDrivers = useCallback(async (search: string = '', page: number = 1) => {
    setLoading(true);
    try {
      let status;
      ...
      const { data } = await axiosAdmin(dispatch).get(`${ApiEndpoints.ADMIN_GET_DRIVERS}?${params}`);
      ...
      toast.error((error as Error).message || `Failed to fetch ${activeTab} drivers`);
      setDrivers([]);
      setPagination(prev => ({
        ...prev,
        currentPage: 1,
  ...
  return (
    <AdminLayout>
      <div className="p-4 md:p-6">
        <h1 className="text-2xl md:text-3xl font-bold mb-6 md:mb-8">Driver Management</h1>
